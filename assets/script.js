...
function fiveDayForcast(coord) {
  fetch(
    `https://api.openweathermap.org/data/2.5/forecast?lat=${coord.lat}&lon=${coord.lon}&appid=${API}&units=imperial`
  )
    .then((response) => response.json())
    .then((apiData) => {
      console.log(apiData);
  ...
  <ul class="list-group list-group-flush">
    ...
    <li class="list-group-item">Humidity:${
      apiData.list[i].weather[0].description
    }</li>
  </ul>
</div>`;
      }
